...
describe('CtripDiscoveryProbe', () => {
  it('页面解析出一家门店时返回 single', async () => {
    ...
    const discoverPromise = probe.discover('persist:xiaozhi:prod:ctrip:aaa');
  ...
  it('页面解析出多家门店时返回 multiple', async () => {
    ...
    const discoverPromise = probe.discover('persist:xiaozhi:prod:ctrip:bbb');
  ...
  it('页面没有解析出任何门店（如 cookie 已过期）时返回 none', async () => {
    ...
    const discoverPromise = probe.discover('persist:xiaozhi:prod:ctrip:ccc');
    electron.views[0].webContents.executeJavaScript.mockResolvedValue([]);
  ...
  it('页面加载或脚本执行异常时返回 none 并记录日志，不抛出', async () => {
    ...
    const discoverPromise = probe.discover('persist:xiaozhi:prod:ctrip:ddd');
  ...
  it('探测完成后无论成功失败都会关闭 WebContentsView', async () => {
    ...
    const discoverPromise = probe.discover('persist:xiaozhi:prod:ctrip:eee');
    const view = electron.views[0];

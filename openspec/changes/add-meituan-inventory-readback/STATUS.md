...
✅ 2026-09-19 补充：日历页与批量页**都已真机验证通过**，两者发的都是形状 A
（`unifiedOperateInvDateModel`）。缺口范围由此收窄到**单房态房量页一个入口**。

---

## ✅ 2026-09-19 联调：端点覆盖面已验完

新增 `端点与回读对照.md` —— 八个端点各自做什么、是否上报、是否回读的查表文档，
起因是「改房态为什么没回读」这个疑问。

| 操作 | 端点 | 回读 | 结果 |
|---|---|---|---|
| 日历页改房量（**含弹窗内顺带改房态**） | `inventory/update` | ✅ | ✅ 通过 |
| 批量改房态房量页 | `inventory/update` | ✅ | ✅ 通过 |
| 单独**开**房 | `inventory/status/switch` | ❌ | ✅ 确认无回读，符合设计 |
| 单独**关**房 | `inventory/roomstatus/submitaudit` | ❌ | ✅ 确认无回读，符合设计 |

### ⚠️ 「改房态没回读」不是缺陷 —— 但两个「改房态」走的是不同端点

美团房态有**两条入口**，结果不同：

```
房价房量日历 → 改房量弹窗 → 顺带改房态   → inventory/update        → ✅ 回读
房价房量日历 → 直接点开/关房              → status/switch 或 submitaudit → ❌ 无回读
```

开/关房不回读的硬原因：报文里只有**单个 `roomId`**（没有房型集合），且日期形状三端点
各不相同（`switch` 给 `startDate`/`endDate`、`submitaudit` 给单个 `date`），
还原不出回读需要的 (房型集 × 日期集)。design 决策 1.1 已排除。

日历页弹窗那条路通过，同时验证了决策 1.1 的另一半：**纯房态的 `countType:1020` 照样回读**，
不按 `countType` 二次分流。

### 📌 遗留判断题（未决）

单独开/关房改变了房态但拿不到回读 cells。**服务端追价若要读 `invSwitch`，这是一条静默
数据空洞**；若只依赖 `status/switch` 的写上报本身则无碍。

需回服务端 ingest 核对 `roomStatus` 与 `inventoryReadback` 两类上报分别怎么消费
—— **本 change 未做此核对**。

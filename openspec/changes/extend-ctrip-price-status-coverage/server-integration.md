...
> ℹ️ 2026-08-21 联调曾发现 `adjustmentPriceOperationsType` 为 `"multiply"` 时返回
> `PARSE_FAILED`，**已修复**。量纲差异见下方 §按比例调价（维护该分支时需要）。
...
desktop 侧报文完整、上报成功（HTTP 200），是服务端未实现 `multiply` 分支 —— **已修复**。
以下量纲说明保留备查，维护该解析分支时需要。

...
**2026-09-21｜代码 + 真机验证完成，已提交（`8c5f53e` / `faa645b` / `408853e`）。
code review 已过（7 条，修 5 留 1 确认 1）。⭐ 8.4 两次通过（房量 + 关房）。**

```
云朵 1834077877   18 商品 → 270 价格格   6 房型 → 90 房态格
三豆  942804505    8 商品 → 120 价格格   7 房型 → 105 房态格
⭐ 浏览器改房量 → 下一轮 changed:1，360 格里精确命中
⭐ 浏览器关房/开房 → 下一轮扫到 invSwitch 1→0→1（2026-09-21 晚补验）
```
...
- **第 9 组 收尾** —— 反向验证（code review 已完成，处理结果见上一节）
- **8.4 的改价那一半仍未验** —— 房量侧已验两次（2026-09-21 晚补了关房/开房：
  云享三人间 09-24 `invSwitch` 1→0→1，20:47:58 那轮扫到）
- 携程 `changed` 非零的原因未排查
- ⏸️ 服务端 `(meituan, inventoryScan)` Translator 未就绪 —— 上报回 `PARSE_FAILED`
  （与携程同状态，`服务端需求.md` 已写好）

### 📌 2026-09-21 晚补充：自然读会写入扫描范围外的钟点房

查库发现同一门店同一天，两条路径写的 `goodsId` 号段完全不同：

```
scan       1135xxx × 18   日租房（-不含早-…）        ← 按 roomCategory 滤掉钟点房
page-read  1179xxx/1194xxx × 6   钟点房（-3小时- / -4小时-）
```

**不是缺陷**，是两条路径覆盖面不同的正常结果：扫描按 `pickMeituanRoomCatalog` 滤钟点房，
自然读照页面返回全收。10-20 那批 page-read 写的就是 `1135xxx` 日租房，两条路是重合的。

⚠️ 它导致 `Inventory scan compared` 的 `cells` 与 `baseline` 恒差 24 格。
**那个差值不是信号** —— 比对以 `latest` 为主循环，基线里多出来的格子不参与任何判断。
日志已改成打 `compared`（`09c1e05`），不再打基线总数。

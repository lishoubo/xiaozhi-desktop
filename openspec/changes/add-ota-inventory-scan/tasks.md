...
- [ ] 7.1 `InventoryScanConfig` 加 `idleMs` 与空闲阈值
- [ ] 7.2 ⚠️ `window.days` 默认值 7 → **15**，与自然读实测范围对齐（design 决策 7）
- [ ] 7.3 ⚠️ 默认**只在开发环境启用**扫描（避免未验证的周期性外部请求进正式包）
- [ ] 7.4 单测：默认值；部分覆盖不影响同组其余项
...
- [ ] 9.7 观察一轮耗时与请求数，定 `idleMs` 默认值，结论回写 design
- [ ] 9.8 如实记录验证结果；未能执行的项说明原因，**不虚构输出**

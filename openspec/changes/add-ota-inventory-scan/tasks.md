...
- [x] 9.1 类型检查 + 受影响模块测试全绿 —— ✅ typecheck 干净；lint 12 个错误与基线
      ...
      本次新增 72 项全绿
- [x] 9.2 真机：不开任何标签页 → 等一轮扫描 → 查库确认 `sourceOfTruth='scan'` 的格子出现
      —— ✅ **2026-09-21 通过**。4 个目标（携程 2 + 美团 2）全部命中，
      `source_of_truth='scan'` 1575 格。实证 `session.fetch` 不依赖标签页。
- [x] 9.3 真机：在渠道后台（**其他浏览器**）改一次房价 → 下一轮扫描应报出差异
      —— 这是本能力的立论场景，必须验
      —— ✅ **2026-09-21 通过两次**（美团云朵 1834077877）：
      ① 改房量：云享三人间 09-21 `limitRemain` 5→7，20:07:43 那轮扫到；
      ② 关房再开房：云享三人间 09-24 `invSwitch/roomStatus` 1→0→1，
      20:47:58 那轮扫到 `content_hash` 从 `0|1|22|1|0|0` 变回 `1|1|22|1|0|1`。
      两次都是**标签页未开**的状态下由定时扫描发现。
- [ ] 9.4 真机：无变化时连续两轮 → 第二轮零上报（不重复报）
      —— ⚠️ 观察到多轮 `changed: 0`（如 20:25:00 那轮），但**未严格构造**
      「同一格连续两轮无变化」的对照，不计入通过
- [ ] 9.5 真机：首轮全新窗口 → 只建基线零上报
- [ ] 9.6 真机：cookie 失效（手动清 partition cookie）→ 跳过该账号 + GlitchTip 有记录
- [ ] 9.7 真机：关掉某家酒店的开关 → 该店不再被扫，同渠道其余店照常
- [x] 9.8 观察一轮耗时与请求数，确认 `idleMs` 默认 5 分钟是否合适；
      若渠道响应慢或有风控迹象则调整，结论回写 design
      —— ✅ **2026-09-21**：4 个目标串行扫完约 **4 秒**（20:56:18→20:56:22），
      无风控迹象。实测轮间隔 5 分 31 秒 / 5 分 6 秒（= `idleMs` + 抖动），符合预期。
      ⚠️ 结论：**dev 也统一成 5 分钟**（原 dev 档 60 秒），见 `21bb119`；
      两档取同值后 `SCAN_PACE` 的环境分档一并取消

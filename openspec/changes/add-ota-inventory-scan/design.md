...
触发风控时，要能不发版就调。⚠️ 实际间隔 = 本轮耗时 + `idleMs` + 抖动，恒大于 5 分钟；
批量页回读实测单次就要 2.9 秒，一轮多账号可能到几十秒。

### 3.1 ⚠️ 随机抖动：让集中部署的机器散开

没有抖动的话，所有客户端按各自启动时刻形成**固定节拍**。同一批装机的机器（集中部署的
门店）会长期保持同相位，每 5 分钟齐刷刷打一次携程 —— 这正是触发风控的形状。

**照抄 `services/updater-service.ts` 的既有手法**（那条链路已在线上跑，见
`RECHECK_BASE_MS` / `RECHECK_JITTER_MS`）：

```ts
const delayMs = idleMs + Math.floor(random() * jitterMs);
...
|---|---|---|
| 基础间隔 | 2 小时 | 5 分钟 |
| 抖动上限 | 1 小时（50%） | **1 分钟（20%）** |

抖动比例取小一些：更新检查是低频动作，散开 1 小时无所谓；扫描要保证对账时效，
散开太多会让「最坏情况多久发现一次变更」变得不可预期。5±1 分钟的最坏情况是 6 分钟，
可接受。

⚠️ **`random` 必须是注入的**（`random?: () => number`），与 `updater-service.ts` 同一
理由：否则「间隔落在预期区间」这条断言不了，测试只能靠跑多次撞概率。

⚠️ 这也是**不能用 `setInterval`** 的第二个理由（第一个是会叠加）：抖动要求每轮间隔都
不同，`setInterval` 只能固定周期。既有实现的注释写明了这点。
...
判断依据只有一条：**认不认识渠道的响应形状**。认识的就是渠道专有。

```
┌─ 渠道无关：一份，加渠道一行不动 ─────────────────────────────┐
│  InventoryScanDispatcher   调度、遍历凭证、开关判定、抖动、   │
│                            比对接线、投递队列 ← 本次新增      │
│  SnapshotWriteQueue        ✅ Change A 已有                   │
│  snapshot-diff             ✅ Change A 已有                   │
│  repository                ✅ Change A 已有                   │
│  AmountChangeReportService ✅ 既有，直接用，无一行新逻辑       │
└──────────────────────────────────────────────────────────────┘
                        ↓ registry 按渠道取实现（照 inventoryReadbacks() 的投影手法）
┌─ 渠道专有：每渠道一份 ───────────────────────────────────────┐
│  InventoryScan 实现        两步请求、请求体六字段形状          │
│  失效判据                  code 表、登录页标记 ← 见下          │
│  行→格子映射               ✅ ctrip-cells.ts 已有（本次复用）  │
│  payload 规格              上报体给 RMS 对接看的说明           │
└──────────────────────────────────────────────────────────────┘
```

⚠️ **失效判据是渠道专有的，不要抽成公共函数** —— 这一条修正 tasks 第 2 节原本
「抽出来复用」的含糊说法。`ctrip/inventory-readback.ts` 里那套判据全是携程形状：

| 判据 | 携程 | 美团 |
|---|---|---|
| 成功码 | `code === 200` | `code === 10000` |
| 失效码表 | `{401, 300, -1}` | 未踩点 |
| 登录页标记 | `htl-ebk-login-web` 等 | 无样本 |

抽成「支持所有渠道所有形态」的参数化函数，会把判据挤回调用方或逼出一个怪物 ——
美团回读那份文件已经记过这个教训（刻意不跨渠道复用 fetcher，同一理由）。

**复用范围限定在携程内部**：扫描与回读共用同一份判据，两者都在 `channels/ctrip/` 下，
抽成同目录的一个模块即可，**不上升到 `channels/` 顶层**。

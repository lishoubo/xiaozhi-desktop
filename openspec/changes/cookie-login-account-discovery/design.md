...
3. 探测成功后创建 —— 按 (channel, otaHotelId) 查重：
   ...
   - 已存在 → 不创建、不动旧账号、不删这次的 partition，只写一条日志，
     具体处理方式（合并/提示/丢弃）留到以后单独做
   - 探测到多个门店 → 弹出列表由用户选择，只为选中的创建账号
```
...
- **[风险] 查重冲突（同一门店被两份不同登录态探测到）目前只记日志，这个处理方式不够**：同一份导入的 cookie 可被多次用于"去登录"，每次都新建一个 partition（一号多店场景下这本是预期行为）；但如果用户两次登录都被探测到同一个门店，就会撞上查重命中的路径——第一次登录创建 `OtaAccount(X)` 关联 `partition-A`，第二次登录新建了 `partition-B`，探测又拿到门店 X，按"只记日志、不处理"，`partition-B` 不会被任何账号关联，永久留存成孤儿；更严重的是，如果此时业务代码正用 `partition-A` 操作门店 X，`partition-B` 也是门店 X 一份完整独立的登录态，"门店 X 当前生效的登录态到底是 A 还是 B"这个问题现有设计没有定义。**这是一个真实的未决问题**，需要在实现前决定：查重命中时是否要把账号的 `partitionName` 更新为最新这次登录的 partition（并想清楚旧 partition 如何处理——删除还是保留为孤儿），还是保持现状但至少需要一个孤儿 partition 的发现/清理机制
...
- **查重命中同一门店时，是否更新账号的 `partitionName` 到最新一次登录的 partition**：见 Risks 中"孤儿 partition"一条。写 Task 4.4（`discover-and-create.ts`）时必须先定这一点再实现，不能沿用"只记日志"这个简化——否则孤儿 partition 和"当前生效登录态不明确"这两个问题会被直接写进代码里。

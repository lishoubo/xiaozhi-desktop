...
页面标签区域 MUST 使用独立的浅灰圆角容器承载标签；标签列表 MUST 可横向滚动，而标签区新建按钮 MUST 固定在该容器右侧，不随标签滚动。当前登录账号区域 MUST 使用紧凑宽度并固定在工作栏右侧。

#### Scenario: 多标签横向溢出

- **WHEN** 当前账号打开的标签宽度超过标签区域可用空间
- **THEN** 页面标签在浅灰容器内横向滚动
- **AND** 标签区新建按钮和当前登录账号区域保持可见
...
### Requirement: 工作区统一控制网页声音

系统 MUST 维护一个仅在当前应用运行期有效的工作区静音状态。账号区声音按钮 MUST 将该状态应用到所有已打开的 OTA 标签；之后新建的任何标签 MUST 继承同一状态，不得按渠道、credential 或 partition 分别维护。

#### Scenario: 静音已有标签

- **WHEN** 用户关闭工作区网页声音
- **THEN** 所有已打开 OTA 标签均被静音
- **AND** 声音按钮显示静音状态

#### Scenario: 静音后新建标签或登录账号

- **WHEN** 工作区处于静音状态并新建任意标签或登录新账号
- **THEN** 新标签在加载页面前继承静音状态

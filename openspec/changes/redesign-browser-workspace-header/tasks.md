...
- [x] 1.16 BrowserManager 增加单一运行期静音状态，状态变化覆盖全部已有标签，新标签在加载前继承。
- [x] 1.17 增加受控的 browser 音频状态读取/设置 IPC 与 preload API。

## 2. 两层头部布局

- [x] 2.1 渠道导航改为 64px 图文入口行，背景 `#F4F6FA`，入口高 40px，并增加 UI 短名称。
- [x] 2.2 页面工作栏改为 64px 三列布局，背景 `#FAFBFC`，浏览器控制、标签区与账号区垂直居中。
- [x] 2.3 标签区支持 132–200px 标签宽度和横向滚动；账号区固定在右侧。
...
- [x] 2.8 标签区增加浅灰圆角底，将标签 `＋` 固定在滚动区右侧，并把账号区收紧为 136–196px / 36px 高。
- [x] 2.9 将账号区喇叭改为可访问的声音开关，并显示当前允许声音/静音状态。
...
- [x] 3.9 定向组件测试覆盖标签区固定 `＋` 的 DOM 结构并通过。
- [x] 3.10 定向测试覆盖已有标签统一静音、新标签继承、IPC 校验和 UI 状态切换。

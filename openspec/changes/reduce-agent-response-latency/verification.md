...
- Duplicate generated-UI render recovery tests passed, including preserving the first valid UI and not hiding unrelated failures.
...
- A second `render_hotel_ui` request after a successful render is stopped before tool execution; the first UI and streamed explanation complete normally instead of ending in `GraphRecursionError`.

...
| 14 | **⭐ 改 `HASH_FIELDS` 不引发误报** | ✅ 移除 `remainCount` 后重启：20:06 首轮 4 个门店全部 `first-round` 跳过（用新 hash 重写基线），20:11 第二轮 `changed` 全 0 |
...
| **售罄跃迁（两渠道）** | ⏳ 库中有**售罄状态**的样本，但没有**跃迁那一刻**的观测 —— 被观测到时已是 0，按设计属持续状态不重复上报。要验需当场卖光或手动压到 0。已知样本：<br>携程 `122244992` 云絮密语房 无早/单早/双早，2026-09-21，总量 4、可售 0、`roomStatus='N'`（卖完→系统自动关房）<br>美团 `942804505` 奢华 雅致双床房，2026-10-02，配额 10、已售 10、`roomStatus=100`（「请补充房型库存」） |
...
`HASH_FIELDS` 在本次**有一处刻意变更**：移除美团的 `remainCount`（预留房量），
它只制造「白比对一场」的噪音。该变更会让既有美团基线全部失效 —— 这在过去是硬禁令，
现在由**基线新鲜度保护**兜住（改字段必然伴随发版 → 重启 → 首轮跳过、只用新 hash
重写基线 → 第二轮起对得上），并已由 #14 实证。携程侧未动。

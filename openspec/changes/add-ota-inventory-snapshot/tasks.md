...
- [x] 10.8 如实记录验证结果 —— 见上。**三项未验，均为有意不验**：

      | 项 | 为何不验 |
      |---|---|
      | 美团 | 本期不接入（design Migration Plan）；`price` 侧 ID 空间未踩点 |
      | `truncated`（勾「应用到所有日期」，携程会改约两年、客户端算不出范围，故只回读窗口内并标记数据不完整） | 属**回读既有链路**，Change A 未改动它；快照侧不消费这个字段 |
      | 失败路径（cookie 失效 / 403。携程失效不保证返回错误码，可能 HTTP 200 + 整页登录页 HTML） | 快照侧的处置是「`JSON.parse` 失败即返回空数组、静默跳过」，失效方式是**少存数据**而非存错数据。代价只是这一轮不更新 |

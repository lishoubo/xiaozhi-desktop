    ...
    >
      ...
      {#if policy === 'agreement'}
        <div class="mt-6 space-y-4 text-sm leading-7 text-secondary-foreground">
          ...
          <p>
            Cookie
            导入由您主动选择文件并授权执行。您应确认对其中账号数据拥有合法使用权。因第三方平台调整、网络故障或不可抗力导致的暂时不可用，我们将在合理范围内协助恢复。
          </p>
          <p>
      ...
      {:else}
        <div class="mt-6 space-y-4 text-sm leading-7 text-secondary-foreground">
          ...
          <p>
            客户端不会主动读取其他浏览器的 Cookie 数据。只有在您选择 JSON 或 Netscape Cookie
            导出文件后才会导入。请避免在公共设备保存敏感凭证。
          </p>
          <p>

...
remote_directory="hotel-butler-upload"
...
artifact_name="hotel-butler-server-deployment-${revision}.tar.gz"
...
ssh "${ssh_options[@]}" "${ssh_target}" \
  "set -eu; cd \"\$HOME/${staging_directory}\"; if command -v sha256sum >/dev/null 2>&1; then sha256sum -c \"${checksum_name}\"; else shasum -a 256 -c \"${checksum_name}\"; fi; mv -f \"${checksum_name}\" \"\$HOME/${remote_directory}/${checksum_name}\"; mv -f \"${artifact_name}\" \"\$HOME/${remote_directory}/${artifact_name}\"; printf '%s\\n' \"${artifact_name}\" >\"\$HOME/${remote_directory}/.current-release.tmp\"; mv -f \"\$HOME/${remote_directory}/.current-release.tmp\" \"\$HOME/${remote_directory}/current-release\"; cd \"\$HOME/${remote_directory}\"; rmdir \"\$HOME/${staging_directory}\""

echo "Production deployment bundle uploaded and verified:"
echo "  server: ${server_ip}"
echo "  remote directory: ~/${remote_directory}"
echo "  artifact: ${artifact_name}"
echo "An existing artifact with the same name was replaced after checksum verification."
echo "No archive was extracted and no service was started."

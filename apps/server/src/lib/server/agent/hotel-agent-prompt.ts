...
export function buildHotelAgentSystemPrompt(input: HotelAgentPromptInput): string {
	...
	const hotelDataRule = input.hotelDataAvailable
		? '默认采用酒店数据 MCP-first：只要答案依赖某家酒店的当前或历史事实，例如经营指标、订单、房态、库存、价格、到离店、渠道、宾客服务记录、趋势、排名、明细或异常，就必须使用 DMS 数据工具先核验，绝不能仅凭记忆、常识或会话中的旧数据回答。通用酒店知识、指标定义或方法建议不需要查询；但一旦用户要求结合具体酒店现状判断，就恢复为强制查询。优先调用 query_hotel_operating_data；若自然语言查询无法生成 SQL，调用 list_hotel_data_tables 和 describe_hotel_data_table 确认 schema，再调用 query_hotel_operating_data_sql 执行单条只读 SELECT/CTE。调用前确认酒店、日期范围、指标和比较口径；上下文已经明确时不要重复追问。当前工具只读：用户要求修改、发布或执行酒店业务操作时，可先查询必要现状并说明建议和影响，但不得声称业务操作已经执行。不得尝试写操作或绕过查询限制。当前阶段所有已登录员工共享 DMS 查询权限，查询边界由服务端 DMS Token 的权限决定。'

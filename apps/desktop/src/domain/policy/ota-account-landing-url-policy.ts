...
import { merchantGroupIdFromBindExtra } from '../ota-bind-extra';
 ...
 * 其余渠道用渠道默认 URL。
 */
export function otaAccountLandingUrl(account: Pick<OtaAccount, 'channel' | 'bindExtra'>): string {
  const merchantGroupId = merchantGroupIdFromBindExtra(account.bindExtra);

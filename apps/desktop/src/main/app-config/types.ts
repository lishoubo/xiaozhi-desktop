...
export type InventoryScanConfig = Readonly<{
  ...
  /**
   * 基线新鲜度上限（毫秒）。**距上次比对超过它，本轮只写基线、不比对、不上报。**
   *
   * ## 解决什么
   *
   * 旧基线与现状的差异，**不代表「渠道刚刚变了」，只代表「这段时间我们没在看」**。
   * 用户关着应用过了一夜，再打开时整批价格都跟昨天不同 —— 那是这一夜里陆续变的，
   * 不是此刻发生的事件。照常比对会在启动后刷一屏假变更（真机实测：隔 17 小时的基线，
   * 一个门店一次报了 205 条）。
   *
   * 与 `snapshot-diff.ts` 对 `added` 的处置**同构**：那边是「基线里没有这一格，不代表
   * 渠道新增了它」，这边是「基线太旧 ≈ 没有基线」。两者都在说**不确定就不报**。
   *
   * ## ⚠️ 代价：每次启动必然漏掉一次对账
   *
   * 上次比对时刻记在**内存**里，重启即丢失，所以启动后第一轮必然跳过。这是**有意的** ——
   * 重启本身就意味着中断过，那一轮的基线年龄不可信；内存方案让「重启后跳过」自动成立，
   * 不需要额外判断。
   *
   * 后果要如实知道：若用户每天只开一次应用，「昨夜渠道自动关房」这类变更**永远不会被
   * 这条链路发现**。取舍是宁可漏报，不要每次启动都刷一屏假变更 —— 而漏掉的只是
   * 「启动前那段时间的变化」，下一轮（`idleMs` 后）即恢复正常对账。
   *
   * ⚠️ 这条要同步给服务端：**desktop 启动后的第一轮不会有上报**，那不是故障。
   */

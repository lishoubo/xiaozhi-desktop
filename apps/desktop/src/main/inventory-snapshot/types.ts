 ...
 *
 * ```
 * page-read   拦截到的读响应       —— 当前唯一在用的来源
 * readback    ⚠️ 当前不产出，见下
 * scan        定时扫描（Change B） —— 覆盖整个配置窗口
 * ```
 *
 * ⚠️ **`readback` 当前不会出现在库里**。回读曾经也写基线，但它发的请求跑在页面上下文里，
 * 必然被改价监听的 CDP 拦到（2026-09-20 真机实证：两次投递相隔 2ms、格子键逐字符相同），
 * 于是同一批数据由 `page-read` 那条路重复写一遍并覆盖标记 —— 纯冗余，已删。
 * 值本身保留：删它要改 DB 的 CHECK 约束（加一个 migration），而 Change B 的主动取数
 * 可能用得上这个语义。
 *

 ...
 * | 云憩大床房（钟点） | 2 | 0 | 999 | 0 |
 *
 * 看着像 `remainCount`=物理剩余、`limitRemain`=限量配额剩余、`usedCount`=已售，
 * 但**没有改前/改后对照的实证** —— 现有踩点的写操作（2026-10-20）与回读样本
 * （2026-09-18~21）日期不重叠，对不上。
 *
 * ⚠️ `limitType: 2`（不限量）时 `limitRemain` 是 **998/999 这类哨兵值**，不是真实房量。
 * 仅凭数字判「还剩几间」会把不限量的房型判成快满房。
 *
 * **desktop 侧整行透传，不取任何一个字段当房量**（与携程同口径）。哪个字段是「用户设的
 * 那个房量」请服务端结合镜像数据确认。
 *
